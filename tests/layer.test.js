...
async function eff(css, contextKey, selector, prop) {
  return resolve(await parseCss(css)).get(contextKey)?.get(selector)?.get(prop)
}

async function propDiff(oldCss, newCss, contextKey, selector, prop) {
  return (await diffCss(oldCss, newCss)).get(contextKey)?.selectors?.get(selector)?.props?.get(prop)
}

describe('@layer: レイヤー順のカスケード', () => {
  it('後に宣言されたレイヤーが通常宣言で勝つ (@layer base, theme;)', async () => {
    const css = `
      @layer base, theme;
      @layer theme { .a { color: blue; } }
      @layer base { .a { color: red; } }
    `
    expect(await eff(css, 'base', '.a', 'color')).toEqual({ value: 'blue', important: false })
  })

  it('非レイヤーの通常宣言はすべてのレイヤーに勝つ', async () => {
    const css = `
      @layer base, theme;
      .a { color: green; }
      @layer theme { .a { color: blue; } }
    `
    expect(await eff(css, 'base', '.a', 'color')).toEqual({ value: 'green', important: false })
  })

  it('!important はレイヤー順を逆転させ、先に宣言されたレイヤーが勝つ', async () => {
    const css = `
      @layer base, theme;
      @layer base { .a { color: red !important; } }
      @layer theme { .a { color: blue !important; } }
    `
    expect(await eff(css, 'base', '.a', 'color')).toEqual({ value: 'red', important: true })
  })

  it('レイヤー内の !important は非レイヤーの通常宣言に勝つ', async () => {
    const css = `
      @layer base;
      .a { color: green; }
      @layer base { .a { color: red !important; } }
  ...
  it('物理的な記述順が同じでも、宣言レイヤー順で勝者が決まる', async () => {
    ...
    const css = `
      ...
      @layer base { .a { color: red; } }
    `
    expect((await eff(css, 'base', '.a', 'color'))?.value).toBe('blue')
  })
})
...
describe('@layer: レイヤー順のみの変更を有効値差分として検出する', () => {
  ...
  it('@layer 宣言順を反転すると .a の color が changed になる (blue → red)', async () => {
    const prop = await propDiff(oldCss, newCss, 'base', '.a', 'color')
    expect(prop?.status).toBe('changed')
    expect(prop?.oldValue).toBe('blue')
    expect(prop?.newValue).toBe('red')
  })

  it('!important のレイヤー順反転も有効値差分として検出する', async () => {
    const body = `
      @layer base { .a { color: red !important; } }
      @layer theme { .a { color: blue !important; } }
    `
    ...
    const prop = await propDiff(oldImp, newImp, 'base', '.a', 'color')
    expect(prop?.status).toBe('changed')
...
describe('@layer: 宣言文なし（暗黙の登録順）', () => {
  it('ステートメントなし: ブロックの出現順が登録順となり後が勝つ', async () => {
    ...
    const css = `
      ...
      @layer base { .a { color: red; } }
    `
    expect(await eff(css, 'base', '.a', 'color')).toEqual({ value: 'red', important: false })
  })

  it('同名レイヤーは最初の出現でのみ登録順が決まる（二度目以降は追記）', async () => {
...
describe('@layer: @media 内の @layer', () => {
  it('@media 内の @layer ルールが @media コンテキストでカスケードされる', async () => {
    const css = `
    ...
    expect(await eff(css, '@media (min-width: 600px)', '.a', 'color'))
      .toEqual({ value: 'blue', important: false })
  })

  it('@media コンテキストの @layer は base コンテキストに影響しない', async () => {
    const css = `
      .a { color: green; }
      @media (min-width: 600px) {
        @layer base { .a { color: red; } }
...
describe('@layer: ネストレイヤー（サブレイヤー）', () => {
  it('@layer 内の @layer は "parent.child" の修飾名で登録される', async () => {
  ...
  it('親レイヤーとサブレイヤーは独立した優先度を持つ', async () => {
