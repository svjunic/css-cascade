...
async function diffCss(oldCss, newCss, diffOptions = {}, parseOptions = {}) {
  const [parsedOld, parsedNew] = await Promise.all([
    parseCss(oldCss, parseOptions),
    parseCss(newCss, parseOptions),
  ])
  return diff(resolve(parsedOld), resolve(parsedNew), diffOptions)
}
...
describe('diff: プロパティの追加・削除・変更', () => {
  it('新しいプロパティが追加される', async () => {
    ...
    expect(prop?.newValue).toBe('0px')
  })

  it('プロパティが削除される', async () => {
    const old = `.a { color: red; margin: 0; }`
    const next = `.a { color: red; }`
    const result = await diffCss(old, next)
    const prop = getPropDiff(result, 'base', '.a', 'margin')
    expect(prop?.status).toBe('removed')
    expect(prop?.oldValue).toBe('0px')
  })

  it('プロパティ値が変更される', async () => {
    const old = `.a { color: red; }`
  ...
  it('!important が付与された場合に changed になる', async () => {
    const old = `.a { color: red; }`
...
describe('diff: @media コンテキスト', () => {
  it('@media 内の変更は @media コンテキストに記録され base に影響しない', async () => {
  ...
  it('@media ブロックが丸ごと追加される場合は added ステータス', async () => {
    const old = `.a { color: red; }`
  ...
  it('ネストした @media の親条件が変わった場合は適用範囲の変更として検出する', async () => {
    const old = `
...
describe('diff: 条件付き at-rule コンテキスト', () => {
  it('@supports 内から base へ移動した宣言を適用範囲の変更として検出する', async () => {
    const old = `
  ...
  it('@container 内から base へ移動した宣言を適用範囲の変更として検出する', async () => {
    const old = `
...
describe('diff: @font-face と @keyframes', () => {
  it('@font-face の src 変更は font face キー配下の changed として検出する', async () => {
    const old = `
      @font-face {
        font-family: "Inter";
        font-weight: 400;
  ...
  it('@keyframes の stop 内プロパティ変更は animation context 内で検出する', async () => {
    ...
    expect(prop?.newValue).toBe('0.8')
  })

  it('@-webkit-keyframes と @keyframes の vendor 差だけでは context 追加削除にならない', async () => {
...
describe('diff: 後勝ちルールが正しく適用された上での比較', () => {
  ...
  it('!important 上書き後の最終値で比較される', async () => {
...
describe('diff: グループセレクタの分解を通じた比較', () => {
  it('グループセレクタが分解されて個別に比較される', async () => {
...
describe('diff: ignoreCosmetic — 表記揺れを無視した比較', () => {
  ...
  it('先頭ゼロ省略 (.2em vs 0.2em) は unchanged になる', async () => {
  ...
  it('負の先頭ゼロ省略 (-.2em vs -0.2em) は unchanged になる', async () => {
    const old = `.a { margin: -0.2em; }`
    const next = `.a { margin: -.2em; }`
    const result = await diffCss(old, next, { ignoreCosmetic: true })
    expect(getPropDiff(result, 'base', '.a', 'margin')?.status).toBe('unchanged')
  ...
  it('calc 内の負の先頭ゼロ省略 (-.5em) は unchanged になる', async () => {
  ...
  it('calc 内で * の後に来る先頭ゼロ省略 (calc(1 * .5em)) は unchanged になる', async () => {
  ...
  it('calc 内で * の後に来る負の先頭ゼロ省略 (calc(1 * -.5em)) は unchanged になる', async () => {
    const old = `.a { margin: calc(1 * -0.5em); }`
    const next = `.a { margin: calc(1 * -.5em); }`
    const result = await diffCss(old, next, { ignoreCosmetic: true })
    expect(getPropDiff(result, 'base', '.a', 'margin')?.status).toBe('unchanged')
  ...
  it('16進カラーの大文字小文字・短縮形は unchanged になる', async () => {
  ...
  it('クォートの有無は unchanged になる', async () => {
  ...
  it('!important の有無は ignoreCosmetic に関わらず changed になる', async () => {
    const old = `.a { color: red; }`
    const next = `.a { color: red !important; }`
    const result = await diffCss(old, next, { ignoreCosmetic: true })
    expect(getPropDiff(result, 'base', '.a', 'color')?.status).toBe('changed')
  ...
  it('カンマ後のスペース有無の違いは unchanged になる', async () => {
  ...
  it('シングルとダブルクォート + カンマスペース違いは unchanged になる', async () => {
  ...
  it('先頭ゼロ省略をショートハンドで使っても unchanged になる', async () => {
    // 例3: margin の値の先頭ゼロ省略
  ...
  it('calc 内の + 直後の先頭ゼロ省略は unchanged になる', async () => {
  ...
  it('calc 内の変数スペース違い (var(--x) * 2 vs var(--x)*2) は unchanged になる', async () => {
    // calc(50%-0.5rem) は Chrome が無効 CSS として拒否するため、valid な CSS で代替する
    const old  = `.a { width: calc(100% - var(--x) * 2); }`
    const next = `.a { width: calc(100% - var(--x)*2); }`
    const result = await diffCss(old, next, { ignoreCosmetic: true })
    expect(getPropDiff(result, 'base', '.a', 'width')?.status).toBe('unchanged')
  })

  it('4桁 hex alpha の短縮形は 8桁へ正規化され unchanged になる', async () => {
...
describe('diff: semanticSelectors — 属性セレクタのクォート等価', () => {
  it('クォートありとなしの属性セレクタが同一セレクタに集約される', async () => {
    const old = `.a [class*='list'] { color: red; }`
    const next = `.a [class*=list] { color: red; }`
    const result = await diffCss(old, next, {}, { semanticSelectors: true })
    // canonical 形 (.a [class*=list]) で unchanged
    const sel = result.get('base')?.selectors?.get('.a [class*=list]')
    expect(sel?.status).toBe('unchanged')
  })

  it('CSSOM は属性セレクタのクォートを自動正規化するため semanticSelectors OFF でも同一セレクタになる', async () => {
    const old = `.a [class*='list'] { color: red; }`
    const next = `.a [class*=list] { color: red; }`
    const result = await diffCss(old, next)
    // CSSOM が両方の selectorText を ".a [class*=list]" に正規化するため 1 セレクタ
    const selectors = [...(result.get('base')?.selectors?.keys() ?? [])]
    expect(selectors.length).toBe(1)
  })

  it('orlfr-article の実例: クォートありとなしが同一に集約される', async () => {
...
describe('diff: スコープ — セレクタ単位の比較（異なるセレクタの DOM オーバーラップは解決しない）', () => {
  ...
  it('.btn に .btn.primary を追加した変更は .btn.primary セレクタの追加として報告される', async () => {
  ...
  it('DOM 上 .btn.primary 要素の実効値変化は「セレクタ追加」としてのみ表現される', async () => {
...
describe('diff: @supports の url() 正規化', () => {
  it('url() 内のコロン（https://）はコロン正規化で破壊されない', async () => {
  ...
  it('url() 内に括弧を含む quoted URL はコロン正規化で破壊されない', async () => {
...
describe('diff: CSS カスタムプロパティ (--var)', () => {
  ...
  it('カスタムプロパティの値変更が changed として検出される', async () => {
  ...
  it('カスタムプロパティの削除が removed として検出される', async () => {
  ...
  it('カスタムプロパティが変わらなければ unchanged', async () => {
...
describe('diff: @font-face', () => {
  it('@font-face の src 変更が changed として検出される', async () => {
  ...
  it('新しいウェイトの @font-face 追加が added として検出される', async () => {
    const old = `@font-face { font-family: 'Roboto'; font-weight: 400; src: url(roboto.woff2); }`
    const next = `
  ...
  it('@font-face が削除されると removed になる', async () => {
    const old = `@font-face { font-family: 'Roboto'; font-weight: 400; src: url(roboto.woff2); }`
    const next = ``
    const result = await diffCss(old, next)
...
describe('diff: @keyframes', () => {
  it('@keyframes のプロパティ変更が changed として検出される', async () => {
  ...
  it('@keyframes の追加が added コンテキストとして検出される', async () => {
    const old = `.a { color: red; }`
  ...
  it('@keyframes の削除が removed コンテキストとして検出される', async () => {
  ...
  it('@keyframes の変更は同名の @keyframes コンテキスト内で記録される', async () => {
...
describe('diff: @font-face font-family 大文字小文字 [バグ]', () => {
  it('font-weight の大小文字差異は同一フォントとして扱う', async () => {
    ...
    const result = await diffCss(oldCss, newCss)
  ...
  it('font-family の大小文字差異は @font-face 内で removed+added でなく changed として扱う', async () => {
