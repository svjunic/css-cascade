...
`)}}</code></pre>
    ...
    </div>`}function Ro(s,e,{highlightHtml:t,showUnchanged:r=!1,contextKey:n="",expanded:i=!1}={}){let{status:l,changeCount:a,props:o}=e,u=t||x(s),c={added:"badge--added",removed:"badge--removed",changed:"badge--changed",unchanged:"badge--unchanged"}[l],p={added:`+${[...o.values()].filter(w=>w.status==="added").length} \u8FFD\u52A0`,removed:`\u2212${[...o.values()].filter(w=>w.status==="removed").length} \u524A\u9664`,changed:(()=>{let w=[...o.values()].filter(E=>E.status==="added").length,L=[...o.values()].filter(E=>E.status==="removed").length,N=[...o.values()].filter(E=>E.status==="changed").length,O=[];return w&&O.push(`+${w}`),L&&O.push(`\u2212${L}`),N&&O.push(`~${N}`),O.join(" ")})(),unchanged:"\u5909\u66F4\u306A\u3057"}[l],f=[...o.entries()].filter(([,w])=>w.status!=="unchanged"),h=[...o.entries()].filter(([,w])=>w.status==="unchanged"),d=[...f.map(([w,L])=>Zn(w,L)),...r?h.map(([w,L])=>Zn(w,L)):[]].join(""),g=h.length>0&&!r?`<button class="unchanged-toggle" data-selector="${x(s)}">
           \u5909\u66F4\u306A\u3057 ${h.length} \u4EF6\u3092\u8868\u793A
         </button>`:"",m=i?Mo(s,e):"",S=i?' data-expanded="true"':"";return`
    <div class="selector-card selector-card--${l}" data-selector="${x(s)}" data-context="${x(n)}"${S}>
      ...
      ${m}
    </div>`}function $o(s,e,t,{showUnchanged:r=!1,expandedSelectors:n=new Set}={}){let{status:i,changeCount:l}=e,o=s==="base"?"\u30C8\u30C3\u30D7\u30EC\u30D9\u30EB (base)":x(s),u={added:"badge--added",removed:"badge--removed",changed:"badge--changed",unchanged:"badge--unchanged"}[i],c=l>0?`<span class="context-badge ${u}">${l} \u4EF6\u306E\u5909\u66F4</span>`:'<span class="context-badge badge--unchanged">\u5909\u66F4\u306A\u3057</span>',p=t?t.map(({selector:h,positions:d})=>({selector:h,positions:d,selDiff:e.selectors.get(h)})).filter(h=>h.selDiff):[...e.selectors.entries()].map(([h,d])=>({selector:h,positions:new Set,selDiff:d}));if(p.length===0)return"";let f=p.map(({selector:h,positions:d,selDiff:g})=>Ro(h,g,{highlightHtml:d.size>0?Po(h,d):null,showUnchanged:r,contextKey:s,expanded:n.has(`${s}||${h}`)})).join("");return`
    ...
    </section>`}function Po(s,e){return[...s].map((t,r)=>{let n=x(t);return e.has(r)?`<mark class="fzf-match">${n}</mark>`:n}).join("")}function Io(s){let e=x(s.prop),t=x(s.oldEffective.value)+(s.oldEffective.important?" !important":""),r=x(s.newEffective.value)+(s.newEffective.important?" !important":"");return`<span class="or-conflict-prop"><span class="or-prop-name">${e}</span>: <span class="or-prop-old">${t}</span> <span class="or-prop-arrow">\u2192</span> <span class="or-prop-new">${r}</span></span>`}function No(s){let e=x(s.oldSelector),t=x(s.newSelector),r=s.conflictingProps&&s.conflictingProps.length>0,n=s.sameSpecificity?'<span class="or-spec-same">\u540C\u4E00\u8A73\u7D30\u5EA6</span>':'<span class="or-spec-diff">\u8A73\u7D30\u5EA6\u304C\u7570\u306A\u308B</span>',i=r?`<div class="or-conflicts">${s.conflictingProps.map(Io).join("")}</div>`:"";return`<tr class="${r?"or-row or-row--moved or-row--conflict":"or-row or-row--moved"}">
    ...
    </tr>`:""}function qo(s,e=!1){let{contextKey:t,rows:r,hasWarning:n}=s,i=t==="base"?"\u30C8\u30C3\u30D7\u30EC\u30D9\u30EB (base)":x(t),l=r.filter(c=>c.type==="moved").length,a=n?`<span class="or-ctx-badge or-ctx-badge--warning">${l} \u4EF6\u306E\u9806\u5E8F\u5909\u66F4</span>`:'<span class="or-ctx-badge or-ctx-badge--ok">\u9806\u5E8F\u5909\u66F4\u306A\u3057</span>',o=`<span class="or-toggle-icon">${e?"\u25BC":"\u25B6"}</span>`,u=r.map(c=>c.type==="moved"?No(c):ko(c)).join("");return`<div class="or-context">
