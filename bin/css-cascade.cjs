...
`)}}</code></pre>
    ...
    </div>`}function No(s,e,{highlightHtml:t,showUnchanged:r=!1,contextKey:n="",expanded:i=!1}={}){let{status:l,changeCount:a,props:o}=e,u=t||x(s),c={added:"badge--added",removed:"badge--removed",changed:"badge--changed",unchanged:"badge--unchanged"}[l],p={added:`+${[...o.values()].filter(w=>w.status==="added").length} \u8FFD\u52A0`,removed:`\u2212${[...o.values()].filter(w=>w.status==="removed").length} \u524A\u9664`,changed:(()=>{let w=[...o.values()].filter(E=>E.status==="added").length,L=[...o.values()].filter(E=>E.status==="removed").length,N=[...o.values()].filter(E=>E.status==="changed").length,O=[];return w&&O.push(`+${w}`),L&&O.push(`\u2212${L}`),N&&O.push(`~${N}`),O.join(" ")})(),unchanged:"\u5909\u66F4\u306A\u3057"}[l],f=[...o.entries()].filter(([,w])=>w.status!=="unchanged"),h=[...o.entries()].filter(([,w])=>w.status==="unchanged"),d=[...f.map(([w,L])=>es(w,L)),...r?h.map(([w,L])=>es(w,L)):[]].join(""),y=h.length>0&&!r?`<button class="unchanged-toggle" data-selector="${x(s)}">
           \u5909\u66F4\u306A\u3057 ${h.length} \u4EF6\u3092\u8868\u793A
         </button>`:"",m=i?Io(s,e):"",S=i?' data-expanded="true"':"";return`
    <div class="selector-card selector-card--${l}" data-selector="${x(s)}" data-context="${x(n)}"${S}>
      ...
      ${m}
    </div>`}function ko(s,e,t,{showUnchanged:r=!1,expandedSelectors:n=new Set}={}){let{status:i,changeCount:l}=e,o=s==="base"?"\u30C8\u30C3\u30D7\u30EC\u30D9\u30EB (base)":x(s),u={added:"badge--added",removed:"badge--removed",changed:"badge--changed",unchanged:"badge--unchanged"}[i],c=l>0?`<span class="context-badge ${u}">${l} \u4EF6\u306E\u5909\u66F4</span>`:'<span class="context-badge badge--unchanged">\u5909\u66F4\u306A\u3057</span>',p=t?t.map(({selector:h,positions:d})=>({selector:h,positions:d,selDiff:e.selectors.get(h)})).filter(h=>h.selDiff):[...e.selectors.entries()].map(([h,d])=>({selector:h,positions:new Set,selDiff:d}));if(p.length===0)return"";let f=p.map(({selector:h,positions:d,selDiff:y})=>No(h,y,{highlightHtml:d.size>0?qo(h,d):null,showUnchanged:r,contextKey:s,expanded:n.has(`${s}||${h}`)})).join("");return`
    ...
    </section>`}function qo(s,e){return[...s].map((t,r)=>{let n=x(t);return e.has(r)?`<mark class="fzf-match">${n}</mark>`:n}).join("")}function To(s){let e=x(s.prop),t=x(s.oldEffective.value)+(s.oldEffective.important?" !important":""),r=x(s.newEffective.value)+(s.newEffective.important?" !important":"");return`<span class="or-conflict-prop"><span class="or-prop-name">${e}</span>: <span class="or-prop-old">${t}</span> <span class="or-prop-arrow">\u2192</span> <span class="or-prop-new">${r}</span></span>`}function Do(s){let e=x(s.oldSelector),t=x(s.newSelector),r=s.conflictingProps&&s.conflictingProps.length>0,n=s.sameSpecificity?'<span class="or-spec-same">\u540C\u4E00\u8A73\u7D30\u5EA6</span>':'<span class="or-spec-diff">\u8A73\u7D30\u5EA6\u304C\u7570\u306A\u308B</span>',i=r?`<div class="or-conflicts">${s.conflictingProps.map(To).join("")}</div>`:"";return`<tr class="${r?"or-row or-row--moved or-row--conflict":"or-row or-row--moved"}">
  ...
  </tr>`}function Uo(s){return s.type==="equal"?`<tr class="or-row or-row--equal">
    ...
    </tr>`:""}function Bo(s,e=!1){let{contextKey:t,rows:r,hasWarning:n}=s,i=t==="base"?"\u30C8\u30C3\u30D7\u30EC\u30D9\u30EB (base)":x(t),l=r.filter(c=>c.type==="moved").length,a=n?`<span class="or-ctx-badge or-ctx-badge--warning">${l} \u4EF6\u306E\u9806\u5E8F\u5909\u66F4</span>`:'<span class="or-ctx-badge or-ctx-badge--ok">\u9806\u5E8F\u5909\u66F4\u306A\u3057</span>',o=`<span class="or-toggle-icon">${e?"\u25BC":"\u25B6"}</span>`,u=r.map(c=>c.type==="moved"?Do(c):Uo(c)).join("");return`<div class="or-context">
  ...
  </div>`}function ts(s,{activeContext:e="all",filterOrderRisk:t=!1,expandedContexts:r=new Set}={}){if(!s||s.length===0)return"";let n=s.reduce((o,u)=>o+u.rows.filter(c=>c.type==="moved").length,0);if(n===0&&!t)return"";let i=t?s.filter(o=>o.hasWarning):s,l=e==="all"?i:i.filter(o=>o.contextKey===e);if(l.length===0)return"";let a=l.map(o=>Bo(o,r.has(o.contextKey))).join("");return`<section class="order-risks-section">
    ...
    ${a}
  </section>`}function rs(s,e,{activeContext:t="all",showUnchanged:r=!1,expandedSelectors:n=new Set}={}){if(!s||s.size===0)return'<div class="empty-state">CSS \u3092\u8AAD\u307F\u8FBC\u3093\u3067\u304F\u3060\u3055\u3044\u3002</div>';let i=null;if(e!==null){i=new Map;for(let a of e)i.has(a.contextKey)||i.set(a.contextKey,[]),i.get(a.contextKey).push({selector:a.selector,positions:a.positions||new Set})}let l="";for(let[a,o]of s){if(t!=="all"&&a!==t)continue;let u=i?i.get(a)||[]:null;i&&u.length===0||(l+=ko(a,o,u,{showUnchanged:r,expandedSelectors:n}))}return l||'<div class="empty-state">\u6761\u4EF6\u306B\u4E00\u81F4\u3059\u308B\u30BB\u30EC\u30AF\u30BF\u304C\u3042\u308A\u307E\u305B\u3093\u3002</div>'}var Go={};function Fo(){return`/* \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
   CSS Diff App \u2014 \u30B9\u30BF\u30A4\u30EB
   \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500 */
...
`;function is(s,e){let t=Fo(),r=[],n=new Set;for(let[u,c]of s)for(let[p,f]of c.selectors)f.status!=="unchanged"&&(r.push({contextKey:u,selector:p,positions:new Set}),n.add(`${u}||${p}`));let i=r.length>0,l=e&&e.length>0,a=e?new Set(e.map(u=>u.contextKey)):new Set,o="";return i&&(o+=`<div class="diff-section">${rs(s,r,{expandedSelectors:n})}</div>`),l&&(o+=ts(e,{expandedContexts:a})),!i&&!l&&(o='<div class="empty-state">\u5DEE\u5206\u306F\u3042\u308A\u307E\u305B\u3093\u3002</div>'),`<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
...
</html>`}var fs=`Usage: css-cascade <old.css> <new.css> [options]
...
Exit codes:
  ...
  2  \u30A8\u30E9\u30FC`,hs;try{hs=(0,cs.parseArgs)({options:{format:{type:"string",default:"text"},filter:{type:"string",default:"changed"},"order-risk":{type:"boolean",default:!1},"ignore-cosmetic":{type:"boolean",default:!1},"semantic-selectors":{type:"boolean",default:!1},"no-color":{type:"boolean",default:!1},version:{type:"boolean",short:"v",default:!1},help:{type:"boolean",short:"h",default:!1}},allowPositionals:!0,args:process.argv.slice(2)})}catch(s){console.error(`Error: ${s.message}`),process.exit(2)}var{values:$,positionals:ps}=hs;$.version&&(console.log("1.0.1"),process.exit(0));$.help&&(console.log(fs),process.exit(0));ps.length<2&&(console.error(`Error: 2\u3064\u306E\u30D5\u30A1\u30A4\u30EB\u30D1\u30B9\u304C\u5FC5\u8981\u3067\u3059
`),console.error(fs),process.exit(2));var Vo=new Set(["text","json","html"]);Vo.has($.format)||(console.error("Error: --format \u306F text | json | html \u306E\u3044\u305A\u308C\u304B\u3067\u3059"),process.exit(2));var Wo=new Set(["changed","added","removed","unchanged","all"]);Wo.has($.filter)||(console.error("Error: --filter \u306F changed | added | removed | unchanged | all \u306E\u3044\u305A\u308C\u304B\u3067\u3059"),process.exit(2));function ds(s){try{return(0,us.readFileSync)(s,"utf8")}catch(e){console.error(`Error: \u30D5\u30A1\u30A4\u30EB\u3092\u8AAD\u307F\u8FBC\u3081\u307E\u305B\u3093 "${s}": ${e.message}`),process.exit(2)}}var[Ko,Ho]=ps,os=ds(Ko),ls=ds(Ho),Pe,ce=[];try{let s={semanticSelectors:$["semantic-selectors"]};Pe=Jn(ue(ae(os,s)),ue(ae(ls,s)),{ignoreCosmetic:$["ignore-cosmetic"]}),$["order-risk"]&&(ce=Zn(os,ls,{semanticSelectors:$["semantic-selectors"]}))}catch(s){console.error(`Parse error: ${s.message}`),process.exit(2)}function as(s,e){return e==="all"?!0:e==="changed"?s!=="unchanged":s===e}function Qo(s){let e=0,t=0,r=0,n=0;for(let[,i]of s)for(let[,l]of i.selectors)for(let[,a]of l.props)a.status==="changed"?e++:a.status==="added"?t++:a.status==="removed"?r++:n++;return{changed:e,added:t,removed:r,unchanged:n}}var q=Qo(Pe),ms=q.changed>0||q.added>0||q.removed>0,gs=$["order-risk"]&&ce.some(s=>s.hasWarning),tt=$.filter;if($.format==="html"){let s=is(Pe,$["order-risk"]?ce:null);process.stdout.write(s),process.exit(ms||gs?1:0)}if($.format==="json"){let s=[];for(let[t,r]of Pe){let n=[];for(let[i,l]of r.selectors){let a=[];for(let[o,u]of l.props)as(u.status,tt)&&a.push({prop:o,...u});a.length>0&&n.push({selector:i,status:l.status,changeCount:l.changeCount,props:a})}n.length>0&&s.push({key:t,status:r.status,changeCount:r.changeCount,selectors:n})}let e={version:1,summary:q,contexts:s};$["order-risk"]&&(e.orderRisks=ce),console.log(JSON.stringify(e,null,2))}else{let s=!$["no-color"]&&!!process.stdout.isTTY,e={reset:s?"\x1B[0m":"",yellow:s?"\x1B[33m":"",green:s?"\x1B[32m":"",red:s?"\x1B[31m":"",cyan:s?"\x1B[36m":"",dim:s?"\x1B[2m":""};for(let[r,n]of Pe){let i=[];for(let[l,a]of n.selectors){let o=[];for(let[u,c]of a.props)as(c.status,tt)&&(c.status==="changed"?o.push(`    ${e.yellow}~${e.reset} ${u}: ${c.oldValue} \u2192 ${c.newValue}`):c.status==="added"?o.push(`    ${e.green}+${e.reset} ${u}: ${c.newValue}`):c.status==="removed"?o.push(`    ${e.red}-${e.reset} ${u}: ${c.oldValue}`):o.push(`      ${u}: ${c.value}`));o.length>0&&(i.push(`  ${e.dim}${l}${e.reset}`),i.push(...o))}i.length>0&&(console.log(`
